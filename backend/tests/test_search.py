...
from backend.index.search import search
...
def test_search_and_ranking():
    with tempfile.TemporaryDirectory() as docs_dir:
        pdf_path = os.path.join(docs_dir, "a.pdf")
        with open(pdf_path, "wb") as f:
            f.write(_make_minimal_pdf_bytes(["Search engines build an index", "Search engines rank results"]))

        docx_path = os.path.join(docs_dir, "b.docx")
        with open(docx_path, "wb") as f:
            f.write(_make_docx_bytes("Search engines index documents with TF-IDF ranking."))

        idx = build_index(docs_dir)

    # Single-term query present in multiple docs
    results = search("search engines", idx)
    # Returns list of (doc_id, score)
    assert isinstance(results, list)
    assert len(results) > 0

    # AND semantics: tokenization removes stopwords; both tokens must exist
    # If either term missing, empty result
    none = search("nonexistentterm anothermissing", idx)
    assert none == []


def test_empty_and_oov_queries():
    with tempfile.TemporaryDirectory() as docs_dir:
        pdf_path = os.path.join(docs_dir, "a.pdf")
        with open(pdf_path, "wb") as f:
            f.write(_make_minimal_pdf_bytes(["Search engines"]))

        idx = build_index(docs_dir)

    assert search("", idx) == []
    assert search("nonexistentterm", idx) == []
